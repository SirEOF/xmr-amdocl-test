...
__kernel void cryptonight(__global ulong *input, __global uint4 *Scratchpad, __global ulong *states, __global uint *Branch0, __global uint *Branch1, __global uint *Branch2, __global uint *Branch3, ulong ThreadCount)
{
	...
	CNKeccak((ulong *)State.hash, inbuf);
	
	//a[0] = State.k[0] ^ State.k[4];
	//b[0] = State.k[2] ^ State.k[6];
	//a[1] = State.k[1] ^ State.k[5];
	//b[1] = State.k[3] ^ State.k[7];
	
	a[0] = ((ulong *)State.hash)[0] ^ ((ulong *)State.hash)[4];
	b[0] = ((ulong *)State.hash)[2] ^ ((ulong *)State.hash)[6];
	a[1] = ((ulong *)State.hash)[1] ^ ((ulong *)State.hash)[5];
	b[1] = ((ulong *)State.hash)[3] ^ ((ulong *)State.hash)[7];
	
	for(uint i = 0; i < 8; ++i) text[i] = vload4(i + 4, (uint *)(State.hash)); //vload4(i, (uint *)(State.init));
	
	for(int i = 0; i < 4; ++i) ((ulong *)ExpandedKey1)[i] = ((ulong *)State.hash)[i];
	for(int i = 0; i < 4; ++i) ((ulong *)ExpandedKey2)[i] = ((ulong *)State.hash)[i + 4];
	...
	for(int i = 0; i < (1 << 17); i += 8)
	{
		for(int j = 0; j < 10; ++j)
		{
			text[0] = AES_Round(AES0, AES1, AES2, AES3, text[0], ((uint4 *)ExpandedKey1)[j]);
			text[1] = AES_Round(AES0, AES1, AES2, AES3, text[1], ((uint4 *)ExpandedKey1)[j]);
			text[2] = AES_Round(AES0, AES1, AES2, AES3, text[2], ((uint4 *)ExpandedKey1)[j]);
			text[3] = AES_Round(AES0, AES1, AES2, AES3, text[3], ((uint4 *)ExpandedKey1)[j]);
			text[4] = AES_Round(AES0, AES1, AES2, AES3, text[4], ((uint4 *)ExpandedKey1)[j]);
			text[5] = AES_Round(AES0, AES1, AES2, AES3, text[5], ((uint4 *)ExpandedKey1)[j]);
			text[6] = AES_Round(AES0, AES1, AES2, AES3, text[6], ((uint4 *)ExpandedKey1)[j]);
			text[7] = AES_Round(AES0, AES1, AES2, AES3, text[7], ((uint4 *)ExpandedKey1)[j]);
		}
	...
	for(int i = 0; i < 0x80000; ++i)
	{
		ulong c[2];
		uint4 a_x = ((uint4 *)a)[0];
		uint4 c_x = Scratchpad[(a[0] & 0x1FFFF0) >> 4];
		
		c_x = AES_Round(AES0, AES1, AES2, AES3, c_x, a_x);
		b_x ^= c_x;
		
		Scratchpad[(as_ulong2(a_x).s0 & 0x1FFFF0) >> 4] = b_x;
		
		uint4 tmp;
		tmp = Scratchpad[(as_ulong2(c_x).s0 & 0x1FFFF0) >> 4];
		
		a[1] += as_ulong2(c_x).s0 * as_ulong2(tmp).s0;
		a[0] += mul_hi(as_ulong2(c_x).s0, as_ulong2(tmp).s0);
		
		Scratchpad[(as_ulong2(c_x).s0 & 0x1FFFF0) >> 4] = ((uint4 *)a)[0];
		
		((uint4 *)a)[0] ^= tmp;
		b_x = c_x;
	}
	
	//for(uint i = 0; i < 8; ++i) text[i] = vload4(i, (uint *)(State.init));
	for(uint i = 0; i < 8; ++i) text[i] = vload4(i + 4, (uint *)(State.hash)); //vload4(i, (uint *)(State.init));
	...
	for(uint i = 0; i < 8; ++i) vstore4(text[i], i + 4, (uint *)(State.hash)); //i, (uint *)(State.init));
	
	keccakf1600((ulong *)State.hash);
		
	states += (25 * (get_global_id(0) - get_global_offset(0)));
	
	for(int i = 0; i < 25; ++i) states[i] = ((ulong *)State.hash)[i];
	
	switch(State.hash[0] & 3)
	{
		case 0:
...
__kernel void Skein(__global ulong *states, __global uint *BranchBuf)
{
...
__kernel void JH(__global ulong *states, __global uint *BranchBuf)
{
...
__kernel void Blake(__global ulong *states, __global uint *BranchBuf)
{
	const uint idx = get_global_id(0) - get_global_offset(0);
	states += 25 * BranchBuf[idx];
	...
	for(int i = 0; i < 4; ++i) states[i] = ((ulong *)h)[i];
}

__kernel void Groestl(__global ulong *states, __global uint *BranchBuf)
{	
	const uint idx = get_global_id(0) - get_global_offset(0);
	states += 25 * BranchBuf[idx];
	...
	for(int i = 0; i < 4; ++i) states[i] = State[i + 4];
}
